...
/// exposed; the crate has zero compile-time dependency on `gix`.
///
/// # Example
///
/// ```
...
impl CommitId {
    ...
    ///
    /// The bytes are stored as-is; callers are responsible for providing
    /// either 20 bytes (SHA-1) or 32 bytes (SHA-256).
    ...
    /// Returns [`CommitIdFromHexError`] for any other input.
    ///
    /// # Example
    ///
    /// ```
...
impl std::fmt::Display for CommitIdFromHexError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "invalid commit id {:?}: expected 40 (SHA-1) or 64 (SHA-256) lowercase hex chars",
            self.0
        )
    }
}
...
/// Which VCS backend a [`Repository`][crate::repository::Repository] uses.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum BackendKind {
    /// Git (via `gix`).
    Git,
    /// Jujutsu (via the `jj` CLI).
    Jj,
}
