...
use crate::{
    types::{CommitId, SortOrder, TagInfo},
    util::{gix_id_to_commit_id, seconds_to_systemtime},
};

const TAGS_PREFIX: &str = "refs/tags/";

pub(super) fn list_tags(repository: &Repository) -> Result<Vec<TagInfo>> {
...
pub(super) fn list_tags_sorted(repository: &Repository, order: SortOrder) -> Result<Vec<TagInfo>> {
...
pub(super) fn create_tag(repository: &Repository, name: &str) -> Result<()> {
...
pub(super) fn create_annotated_tag(
    ...
    let tagger = repository
        .committer()
        .ok_or_else(|| {
            anyhow::anyhow!("no committer identity — set user.name and user.email in git config")
        })?
...
pub(super) fn delete_tag(repository: &Repository, name: &str) -> Result<()> {
    ...
    let edit = gix::refs::transaction::RefEdit {
        ...
        name: full_ref.as_str().try_into().map_err(|_| {
            anyhow::anyhow!("invalid ref name '{}'", full_ref)
        })?,
        deref: false,
    };
    repository
